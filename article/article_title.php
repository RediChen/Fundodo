...
<body>
  <div class="container">
    ...
    <ul class="nav nav-tabs mb-2">
      <li class="nav-item">
        <a class="nav-link <?php if (!isset($_GET["sort"])) echo "active" ?> text-primary-emphasis hover" href="article_title.php?page=1">所有文章</a>
      </li>

      <?php foreach ($sort_rows as $title_sort) : ?>
        <li class="nav-tiem">
          <a class="nav-link <?php if (isset($_GET["sort"]) && $sort == $title_sort["id"]) echo "active"; ?> text-primary-emphasis hover" href="article_title.php?sort=<?= $title_sort["id"] ?>&page=1"><?= $title_sort["sort"] ?></a>
        </li>
      <?php endforeach; ?>
      ...
      <table class="table table-bordered">
        ...
        <tbody>
          <?php foreach ($rows as $article) : ?>
            <tr class="text-nowrap">
              <td>
                <a href="article_title.php?sort=<?= $article["sort"] ?>&page=1" class="text-primary-emphasis hover">
                  <?= $article["arti_sort"] ?>
                </a>
              </td>
              <td>
                <div class="arti-title"><a href="article_content.php?Aid=<?= $article["id"] ?>" class="text-primary-emphasis hover">
                  <?= $article["title"] ?>
                </a></div>
              </td>
